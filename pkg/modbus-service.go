...
func (s *ModbusService) ReadHoldingRegisters(address uint16, quantity uint16) ([]uint16, error) {
	...
	registers := make([]uint16, len(response)/2)
	for i := 0; i < len(response); i += 2 {
		registers[i/2] = uint16(response[i])<<8 | uint16(response[i+1])
	}
